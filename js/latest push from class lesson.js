...
class ToDo {
  ...
  // look at notes from tuesday 3/3 
  // or space battle
  markComplete() {
    // this method should just change the value of the boolean
    this.completed = true    
    // console.log("markComplete called for " + this.itemContent); // just to help make it easier
  }
...
// 0. get your code caught up to this commit

// 1. we now have an X in a span on each item.  add a single-click listener to ul#todo-list
// figure out if the person clicked the X or not in the handler using event.target
// hint check if the class list contains "delete-item" (google that)

// 2. in the handler make sure you can log the item index of the thing you clicked the X 
// next to 
// hint: .parentNode

// 3. once you have the index logging, add a function app.deleteTodo()
// should this function take an argument? 
// what should this function do?
// call app.deleteTodo from the handler -- should you pass something in?

// hint -- only call app.deleteTodo IF the classlist contains "delete-item"
// ie. if the user clicked the X

// bonus -- style the X to look like a link even thought it's not a link
// fun pro-tip: check out CSS property cursor
